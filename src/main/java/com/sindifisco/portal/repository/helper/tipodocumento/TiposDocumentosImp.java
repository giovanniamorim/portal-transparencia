...
public class TiposDocumentosImp implements TiposDocumentosQueries{
	...
	public Page<TipoDocumento> filtrar(TipoDocumentoFilter filtro, Pageable pageable) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(TipoDocumento.class);
	...
	private Long total(TipoDocumentoFilter filtro) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(TipoDocumento.class);
		adicionarFiltro(filtro, criteria);
	...
	private void adicionarFiltro(TipoDocumentoFilter filtro, Criteria criteria) {
		if(filtro != null) {
