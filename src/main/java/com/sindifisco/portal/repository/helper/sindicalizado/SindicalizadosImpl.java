...
public class SindicalizadosImpl implements SindicalizadosQueries{
	...
	public Page<Sindicalizado> filtrar(SindicalizadoFilter filtro, Pageable pageable) {
		...
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Sindicalizado.class);
	...
	private Long total(SindicalizadoFilter filtro) {
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Sindicalizado.class);
		adicionarFiltro(filtro, criteria);
	...
	private void adicionarFiltro(SindicalizadoFilter filtro, Criteria criteria) {
		if(filtro != null) {
			...
			if (!StringUtils.isEmpty(filtro.getCpf())) {
				criteria.add(Restrictions.ilike("cpf", filtro.getCpf(), MatchMode.START));
			}
	...
	public Sindicalizado buscarComCidadeEstado(Long codigo) {
		...
		return (Sindicalizado) criteria.uniqueResult();
	}
